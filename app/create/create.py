...
async def create_page(pages: Annotated[str, typer.Option(help="JSON string of the pages to create.")] = "",
...
async def create_command(commands: Annotated[str, typer.Option(help="JSON string of the commands to create.")] = "",
                         page_id: Annotated[int, typer.Option(help="Id of the page to add the commands.")] = -1):
    """ Attempts to create the commands given."""
    page_ids = [str(i) for i in await fetch_page_ids()]
    if commands == "":
        commands = []
        if page_id == -1:
            page_id = int(Prompt.ask("Enter the page id ", choices=page_ids, default=page_ids[0]))
        ...
        commands.append(Command.model_validate({"name": name, "text": text, "page": page}))
    else:
