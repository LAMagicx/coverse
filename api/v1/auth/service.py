...
def login(request: Request, auth_details: AuthDetails):
    if auth_details.username == USERNAME and request.app.auth.verify_password(
        auth_details.password, HASHED_PASSWORD
    ):
        # login correct
        token = request.app.auth.encode_token(USERNAME)
        logger.log(
            "EVENT", f"LOGIN user: {auth_details.username} - {request.client.host}"
        )
        return {"token": token}
    else:
        logger.exception(
            f"LOGIN FAILED user: {auth_details.username} - {request.client.host} : bad password"
        )
